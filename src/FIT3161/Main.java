...
import edu.monash.fit.eduard.grid.operator.ClampToRangeOperator;
import edu.monash.fit.eduard.grid.operator.CopyOperator;
import edu.monash.fit.eduard.grid.operator.GradientOperator;
import edu.monash.fit.eduard.grid.operator.GridOperator;
import edu.monash.fit.eduard.grid.operator.LowPassOperator;
import edu.monash.fit.eduard.utils.FileUtils;
import edu.monash.fit.eduard.grid.operator.GridToImageOperator;
import edu.monash.fit.eduard.grid.operator.PlanCurvatureZevenbergenThorneOperator;
import edu.monash.fit.eduard.grid.operator.ScaleToRangeOperator;
import edu.monash.fit.eduard.grid.operator.ShadingOperator;
import edu.monash.fit.eduard.grid.operator.ThreadedGridOperator;
import edu.monash.fit.eduard.grid.operator.WeightedScaleOperator;
import edu.monash.fit.eduard.grid.operator.lic.LineIntegralConvolutionOperator;
...
/**
 * A sample application for FIT3161 Semester 2 - 2019 to demonstrate reading and
 ...
 */
public class Main {
    // path to the cuda script
    private static String cudaPath = null;
    ...
    private static void mainInEventDispatchThread() {
        try {
            ...
            // import cuda script
            if (cudaPath == null) {
                cudaPath = FileUtils.askFile(null, "Select your cuda script", true);
            }
            if (cudaPath == null) {
                // user canceled
                return;
            ...
            System.out.println("Cuda script: " + cudaPath);

            // setup filter and apply filter to grid
            System.out.println("Filtering");
            CommandLine cmdLine = CommandLine.parse("nvcc -o filter_acceleration.out "+ '"'+cudaPath+'"');
            Executor executor = new DefaultExecutor();
            DefaultExecuteResultHandler resultHandler = new DefaultExecuteResultHandler();
            // end the process when the JVM exits
            File directory = new File(FileUtils.getParentDirectoryPath(cudaPath));
            ...
            if (executor.isFailure(SUCCESS_EXIT_CODE)==false){
                cmdLine = CommandLine.parse(FileUtils.getParentDirectoryPath(cudaPath)+"filter_acceleration.out " +'"'+ inputFilePath+'"');
                // run the child process
                executor = new DefaultExecutor();
                executor.setWorkingDirectory(directory);
                DefaultExecuteResultHandler newResultHandler = new DefaultExecuteResultHandler();
                ...
                // run the child process
                executor.execute(cmdLine, newResultHandler);
                // block this thread until the child process has finished
                newResultHandler.waitFor();
                
                if (executor.isFailure(SUCCESS_EXIT_CODE)==false){
                    outputFilePath = FileUtils.getParentDirectoryPath(cudaPath)+"out.asc";
                    Grid filteredGrid = EsriASCIIGridImporter.read(outputFilePath);
                    ...
                    // shade the filtered grids and export to images
    ...
    // The main bulk that needs to be parallelised
    // Code uses a lot of Math libraries
